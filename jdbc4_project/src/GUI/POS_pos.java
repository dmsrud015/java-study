...
public class POS_pos extends JPanel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        String stockprice = ItemDAO.getInstance().getprice(item);
        String stock = ItemDAO.getInstance().getStock(item);
        ...
        if (obj == buttonItemAdd) {//결제버튼 클릭
        ...
        } else if (obj == buttonCancer) {//취소버튼클릭
            int result;
            result = JOptionPane.showConfirmDialog(null, "주문을 취소 하시겠습니끼?");
            //���� ��������
            if (result == 0) {
                clear();
            }
            ...
            if (jTextFieldTotal.getText().equals("") || jTextFieldTotal.getText().equals(null)) {
                ...
                //���� ��������
                if (result == 0) {
                    input = Integer.parseInt(JOptionPane.showInputDialog("총금액은" + total + "입니다.\n얼마를 지불하시겠습니까?"));
                    System.out.println(input);

                    if (input >= total) {
                        res = input - total;
                        JOptionPane.showMessageDialog(null, "지불하신금액은" + input + "\n상품의 합계는" + total + "\n거스름돈은" + res + "입니다");

                        System.out.println(res);
                        stock_update(model);
                    }
                } else {
                    JOptionPane.showMessageDialog(null, "결제를 취소였습니다");
